...
class DashboardRqtPlugin(Plugin):
    ...
    def check_expected_windows(self):
        ...
        try:
            ...
            # Auto press buttons if program windows are already running
            for window_name, window_info in windows_status.items():
                if window_name not in ALL_WINDOW_COMMANDS:
                    continue

                btn = self.bnl.get(f"{window_name}_push")

                if (
                    window_name not in self.expected_active_windows
                    and window_info.get("has_process", False)
                    and btn is not None
                ):
                    self.expected_active_windows.append(window_name)
                    btn.setChecked(True)
